...
func day11(example int) {
	colors := make(map[point]bool)
	colors[point{0, 0}] = false
	execute11(colors)
	fmt.Println("D11-P1:", len(colors))

	colors = make(map[point]bool)
	colors[point{0, 0}] = true
	min, max := execute11(colors)
...
func execute11(colors map[point]bool) (point, point) {
	...
	for !isDone {
		...
		case newPaint := <-output:
			...
			position = moveRobot(position, heading)
			min = pointMin(position, min)
			max = pointMax(position, max)
		case _ = <-*ic.requestInput:
			if colors[position] {
				input <- int64(1)
			} else {
				input <- int64(0)
...
func moveRobot(pos point, head int) point {
	switch head {
	case 0:
		pos.y++
	case 1:
