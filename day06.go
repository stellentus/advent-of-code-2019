...
func init() {
	// Originally implemented at https://play.golang.org/p/QjiOYZgnT3v
	codeForDay[6] = day6
}
...
func treeOfPlanets(pairs []pair) (*planet, *planet) {
	dict := make(map[string]*planet)
	populate(dict, pairs)
	calcDist(planetFor(dict, "YOU"), 1)
	return planetFor(dict, "COM"), planetFor(dict, "SAN")
}

func populate(dict map[string]*planet, pairs []pair) {
	for _, pr := range pairs {
		c := planetFor(dict, pr.c)
		o := planetFor(dict, pr.o)

		c.kids = append(c.kids, o)
		newDesc := o.numDesc + 1
		c.numDesc += newDesc
		newOrb := o.numOrbs + o.numDesc + 1
		c.numOrbs += newOrb
		addOrbits(c.par, newDesc, newOrb, 0) // Now go up c's parents to add to their numDesc and numOrbs

		o.par = c
	}
}

func planetFor(dict map[string]*planet, nm string) *planet {
	pl, ok := dict[nm]
	if !ok {
		pl = &planet{name: nm, distYou: math.MaxInt32}
		dict[pl.name] = pl
	}
	return pl
}

func addOrbits(pl *planet, newDesc, newOrb, layers int) {
	if pl == nil {
		return
	}

	pl.numDesc += newDesc
	pl.numOrbs += newOrb + newDesc
	addOrbits(pl.par, newDesc, newOrb+newDesc, layers+1)
}

func (p planet) connections() []*planet {
	if p.par == nil {
		return p.kids
	} else {
		return append(p.kids, p.par)
	}
}

func calcDist(plan *planet, dist int) {
	for _, con := range plan.connections() {
		if con.distYou > dist {
			con.distYou = dist
			calcDist(con, dist+1)
		}
